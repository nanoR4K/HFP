...
def max_min_Angle(caseComb, time, currentDataFrame):
    t = time
...
surf_Angles = casefoam.posField_to_timeSeries(
    surfaceDir, 'p_freeSurface.raw', max_min_Angle, caseStructure, baseCase)
surf_Angles = surf_Angles.reset_index()

plt.figure()
sns.lineplot(x='time',y='max',hue='var_0',style='var_1',data=surf_Angles)
plt.xlabel('x [m]')
plt.ylabel('h [m]')
plt.show()
